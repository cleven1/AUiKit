...
@IBDesignable public class AUiTabs: UIControl {
    ...
    public var onScrollChangeClosure: ((Int) -> Void)?
    public var onDidItemClosure: ((Int) -> Void)?
    private var titleLabels: [UILabel] = []
    private var preLabel: UILabel?
    private var isOnClickTitle: Bool = false
    ...
    private var isScrollRight: Bool = false
    private var lastContentOffsetX: CGFloat = 0
    /// 实时滚动
    public func scrollToPosition(point: CGPoint) {
        guard isOnClickTitle == false else { return }
        var page = selectIndex
        if point.x < lastContentOffsetX {
            page = selectIndex - 1
            isScrollRight = true
        } else if point.x > lastContentOffsetX {
            page = selectIndex + 1
            isScrollRight = false
        }
        if titleLabels.isEmpty { return }
        page = page >= titleLabels.count ? titleLabels.count - 1 : page < 0 ? 0 : page
        let currentLabel = titleLabels[page]
        var rect = indicator.frame
        let margin = style.titlePendingHorizontal * 2.0 + style.titleMargin * CGFloat(titleLabels.count)
        let w: CGFloat = scrollView.frame.width + margin
        let scale = point.x / w - CGFloat(page)
        if isScrollRight {
            let x = (preLabel?.frame.minX ?? 0) - currentLabel.frame.width * 0.5 - (currentLabel.frame.width - currentLabel.frame.width * scale)
            rect.origin.x = x > rect.origin.x ? rect.origin.x : x
        } else {
            let x = (preLabel?.frame.maxX ?? 0) + currentLabel.frame.width * scale
            rect.origin.x = x < rect.origin.x ? rect.origin.x : x
        }
        setIndicatorFrame(rect)
        lastContentOffsetX = point.x
    }
    
    /// 滚动到指定位置
    public func scrollToPage(point: CGPoint) {
        if isOnClickTitle {
            isOnClickTitle = false
            return
        }
        if scrollView.frame.width <= 0 { return }
        var page = Int(point.x / scrollView.frame.width)
        page = page < 0 ? 0 : page >= titleLabels.count ? titleLabels.count - 1 : page
        setSelectIndex(index: Int(page), animated: true)
    }
